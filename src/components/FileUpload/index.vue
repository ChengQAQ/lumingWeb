  ...
  export default {
          ...
          if (val) {
            ...
            this.fileList = list.map(item => {
              if (typeof item === "string") {
                // 从映射表中获取原始文件名，如果没有则使用 URL
                const originalName = this.urlToNameMap[item] || item
                item = { name: originalName, url: item }
              }
    ...
    methods: {
      ...
      handleUploadSuccess(res, file) {
        ...
        if (res.code === 200) {
          // 保存 URL 到原始文件名的映射
          this.urlToNameMap[res.fileName] = file.name
      ...
      getOriginalFileName(url) {
        return this.urlToNameMap[url] || url
      },
      // 获取 URL 到原始文件名的映射表
      getUrlToNameMap() {
        return this.urlToNameMap
      }
    }
  }
  </script>
