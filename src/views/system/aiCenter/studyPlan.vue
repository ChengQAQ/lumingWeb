<template>
  <div class="app-container">
    ...
    <div class="page-header">
      <h2>AI学案生成</h2>
      <p class="page-description">上传课件文件，AI将为您生成专业的学案</p>
    </div>
      ...
      <div class="right-panel">
        ...
        <div class="preview-content">
          ...
          <div v-else class="empty-state">
            <el-empty description="请上传文件并生成学案"></el-empty>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <el-form :model="saveForm" :rules="saveRules" ref="saveForm" label-width="120px">
        <el-form-item label="*文件用途" prop="filePurpose">
          <el-input v-model="saveForm.filePurpose" placeholder="学案" disabled />
        </el-form-item>
...
<script>
import { convertStudyplan } from "@/api/system/aiCenter"
...
export default {
  ...
  data() {
    return {
      ...
      toolbars: {
        ...
        quote: true, ol: true, ul: true, link: true, imagelink: true,
        code: true, table: true, help: true, save: false
      },
      xssOptions: {
        whiteList: {
          h1: [], h2: [], h3: [], h4: [], h5: [], h6: [],
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      this.$message.success('文件选择成功，可以开始生成学案')
      return false
    },
    handleUploadSuccess(response, file) { this.$message.success('文件上传成功'); this.selectedFile = file },
    handleUploadError() { this.$message.error('文件上传失败') },
    handleFileChange(file) { if (file && file.raw) { this.selectedFile = file.raw; this.$message.success('文件选择成功，可以开始生成学案') } },
    ...
    async generateStudyPlan() {
      ...
      try {
        ...
        this.$message({ message: 'AI正在生成学案，请耐心等待...', type: 'info', duration: 0, showClose: false })
        // 创建取消源
        const CancelToken = (await import('axios')).default.CancelToken
        this.cancelSource = CancelToken.source()
        const response = await convertStudyplan(formData, { cancelToken: this.cancelSource.token })
        ...
        if (response.status === 'success' && response.markdown) {
          ...
          this.$message({ message: '学案生成成功', type: 'success', showClose: true })
          this.saveToHistory({ id: Date.now(), fileName: this.selectedFile.name, markdown: response.markdown, provider: this.selectedProvider, createTime: new Date().toISOString() })
        } else {
          this.$message.error(response.msg || '生成失败')
        }
      } catch (error) {
        this.$message.closeAll()
        if (error.message && error.message.includes('timeout')) this.$message.error('生成超时，请稍后重试或联系管理员')
        else if (error.message && error.message.includes('Network Error')) this.$message.error('网络连接异常，请检查网络后重试')
        else this.$message.error('生成失败：' + (error.message || '网络错误'))
    ...
    async downloadDocx() {
      if (!this.currentMarkdown) return
      const latest = this.isEditing ? this.editingMarkdown : this.currentMarkdown
      const blob = await convertMarkdownToDocxBlob(latest)
      const def = `${this.selectedFile?.name?.replace(/\.[^/.]+$/, '') || '学案'}_学案`
      showFileNameDialog(def, (name) => { saveAs(blob, `${name}.docx`); this.$message.success('文件下载成功') }, this)
    },

    // 保存到我的文件
    saveToMyFiles() { if (!this.currentMarkdown) return; this.saveForm.userFname = `${this.selectedFile?.name?.replace(/\.[^/.]+$/, '') || '学案'}_学案`; this.saveForm.filePurpose = '学案'; this.saveDialogVisible = true },
    async confirmSave() {
      this.$refs.saveForm.validate(async (valid) => {
        ...
        try {
          const latest = this.isEditing ? this.editingMarkdown : this.currentMarkdown
          const docxBlob = await convertMarkdownToDocxBlob(latest)
          let knowledgeValue = ''
          if (this.saveForm.knowledge && this.saveForm.knowledge.length > 0) knowledgeValue = Array.isArray(this.saveForm.knowledge) ? this.saveForm.knowledge.join('/') : this.saveForm.knowledge
          const formData = new FormData()
          ...
          const response = await request({ url: '/system/teacher/addFile', method: 'post', data: formData, headers: { 'Content-Type': 'multipart/form-data' } })
          if (response.code === 200) { this.$message.success('保存成功'); this.saveDialogVisible = false; this.resetSaveForm() } else { this.$message.error(response.msg || '保存失败') }
        } catch (e) { this.$message.error('保存失败：' + (e.message || '网络错误')) } finally { this.saving = false }
      })
    },
    resetSaveForm() { this.saveForm = { filePurpose: '学案', userFname: '', subjectName: '', knowledge: '' } },

    // 历史记录
    loadHistoryFromStorage() { const history = localStorage.getItem('studyPlanHistory'); this.historyList = history ? JSON.parse(history) : [] },
    saveToHistory(item) { this.historyList.unshift(item); if (this.historyList.length > 20) this.historyList = this.historyList.slice(0, 20); localStorage.setItem('studyPlanHistory', JSON.stringify(this.historyList)) },
    ...
    deleteHistory(item) { this.$confirm('确定要删除这条历史记录吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => { const i = this.historyList.findIndex(h => h.id === item.id); if (i > -1) { this.historyList.splice(i, 1); localStorage.setItem('studyPlanHistory', JSON.stringify(this.historyList)); this.$message.success('删除成功') } }) },
