<template>
  <div class="app-container">
    <div class="page-header">
      <h2>备课材料库</h2>
      <p class="page-description">管理备课相关的教学材料</p>
    </div>
  ...
  data() {
    return {
      ...
      uploadRules: {
        ...
        teachingIds: [
          { required: true, message: '请选择备课组', trigger: 'change' }
        ]
      },
    ...
    isPptxFile() {
     ...
     convertSubjectToCode(subjectName) {
       ...
       return subjectMap[subjectName] || null;
              ...
              // 加载备课组列表
       loadGroupList() {
         console.log('开始加载备课组列表...')
         listGroup1().then(response => {
           console.log('备课组API响应:', response)
           if (response.code === 200) {
             // 过滤只显示groupType为1的备课组
             this.groupList = (response.rows || []).filter(group => group.groupType === 1)
           } else {
           ...
           if (memberResponse.code === 200 && memberResponse.rows && memberResponse.rows.length > 0) {
             ...
             Promise.all(groupPromises).then(groupsWithDetails => {
               ...
               const userGroups = validGroups.filter(member => 
                 member.groupType === 1 && member.groupId
               )
               
               console.log('过滤后的备课组:', userGroups)
               
               if (userGroups.length > 0) {
                 console.log('用户所属的备课组:', userGroups)
