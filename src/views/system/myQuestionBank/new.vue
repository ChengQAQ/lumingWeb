    ...
    >
      <el-table-column label="科目" align="center" prop="subject" width="150">
        ...
        <template slot-scope="scope">
          <el-tag type="primary" size="small">{{ getSubjectName(scope.row.subjectCode || scope.row.subject) }}</el-tag>
        </template>
      </el-table-column>
        ...
        <template slot="header" slot-scope="scope">
          <div class="table-header-filter">
            ...
            >
              ...
              <el-option label="组卷" :value="1" />
              <el-option label="作业" :value="2" />
            </el-select>
          </div>
        </template>
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type === 1" type="success" size="small">组卷</el-tag>
          <el-tag v-else-if="scope.row.type === 2" type="primary" size="small">作业</el-tag>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="题目数量" align="center" width="150">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.questionCount" type="info" size="small">
            {{ scope.row.questionCount }}题
          </el-tag>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="创建人" align="center" prop="operateBy" width="200" >
        <template slot="header" slot-scope="scope">
          <div class="table-header-filter">
            <el-select
          ...
          <div class="detail-title-section">
            ...
            <div class="detail-meta-tags">
              <el-tag type="primary" size="medium" class="meta-tag">
                ...
                {{ getSubjectName(currentItem.subjectCode || currentItem.subject) }}
              </el-tag>
    ...
    >
      <el-form ref="updateForm" :model="updateForm" :rules="getUpdateRules()" label-width="100px">
        ...
        <el-form-item label="题目列表">
          <div class="update-question-list">
            <div v-if="updateQuestionDetails.length === 0" class="no-questions">
              暂无题目
            </div>
            <div v-else class="question-items">
              <div
                v-for="(question, index) in updateQuestionDetails"
...
<script>
import { listPaperAssignment, getPaperAssignment, delPaperAssignment, addPaperAssignment, updatePaperAssignment } from "@/api/system/paperAssignment"
import { getQuestionsBySids, htmlToWord, getQuestionDetail } from "@/api/system/paper"
...
import QuestionAnalysisDialog from './components/QuestionAnalysisDialog'
import QuestionEditDialoq from './components/QuestionEditDialoq'
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        subjectCode: '', // 默认为空字符串，表示"全部"（新接口使用 subjectCode）
        customPaperName: '', // 使用空字符串，el-input 需要字符串类型
        creator: '', // 默认为空字符串，表示"全部"
        type: '', // 类型筛选：''-全部，1-组卷，2-作业
      ...
      updateQuestionDetails: [], // 修改弹窗中的题目详情列表
      updateLoading: false, // 修改提交加载状态
      // 收藏相关数据
  ...
  created() {
    ...
    // 检查路由查询参数，如果有类型参数则设置当前类型（只支持 homework）
  ...
  watch: {
    ...
    '$route'(to, from) {
      ...
      if (to.path === '/system/myQuestionBank/index' || to.path === '/system/myQuestionBank/new') {
        // 检查是否有类型参数，只支持 homework
        if (to.query && to.query.type) {
          const targetType = to.query.type
          if (targetType === 'homework' && this.currentType !== targetType) {
            this.currentType = targetType
          }
        }
        // 延迟检查，确保 store 状态已更新
        this.$nextTick(() => {
          // 检查是否需要刷新列表（从麓鸣题库跳转过来时）
          this.checkAndRefresh()
        })
      }
    }
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        subjectCode: this.queryParams.subjectCode === '' ? null : this.queryParams.subjectCode,
        ...
        type: this.queryParams.type === '' ? null : (this.queryParams.type === 1 || this.queryParams.type === '1' ? 1 : (this.queryParams.type === 2 || this.queryParams.type === '2' ? 2 : Number(this.queryParams.type))),
    ...
    handleUpdate(row) {
      ...
      getPaperAssignment(id).then(response => {
        if (response.code === 200) {
          const mainData = response.data.main || response.data
          const details = response.data.details || []

          this.updateForm = {
            id: mainData.id,
            customPaperName: mainData.customPaperName
          }

          // 初始化题目详情列表，用于编辑
          this.updateQuestionDetails = details.map(detail => ({
            id: detail.id, // 子表ID，修改时必填
            sid: detail.sid, // 题目ID
            questionType: detail.questionType || '未知题型', // 题型
            score: detail.score !== null && detail.score !== undefined ? Number(detail.score) : 0 // 分值
          }))
    ...
    submitUpdate() {
      this.$refs.updateForm.validate(valid => {
        if (valid) {
          // 先获取当前数据，获取主表信息
          this.updateLoading = true
          getPaperAssignment(this.updateForm.id).then(response => {
            if (response.code === 200) {
              const mainData = response.data.main || response.data

              // 构建修改数据，使用用户编辑后的 questionDetails
              const formData = {
                id: this.updateForm.id,
                customPaperName: this.updateForm.customPaperName,
                subjectCode: mainData.subjectCode,
                type: mainData.type,
                questionDetails: this.updateQuestionDetails.map(detail => ({
                  id: detail.id, // 子表ID，修改时必填
                  sid: detail.sid, // 题目ID
                  questionType: detail.questionType || '未知题型', // 用户编辑后的题型
                  score: detail.score !== null && detail.score !== undefined ? Number(detail.score) : 0 // 用户编辑后的分值
                }))
              }

              return updatePaperAssignment(formData)
            } else {
    ...
    handleDelete(row) {
      const id = row.id
      this.$modal.confirm(`是否确认删除作业编号为"${id}"的数据项？`).then(() => {
        return delPaperAssignment(id)
      }).then(() => {
        this.getList()
    ...
    loadQuestionDetails() {
      this.loadingDetail = true
      getPaperAssignment(this.currentItem.id).then(response => {
        if (response.code === 200) {
          const mainData = response.data.main || response.data
          const details = response.data.details || []

          // 更新 currentItem 以包含主表数据
          this.currentItem = { ...this.currentItem, ...mainData }

          if (details && details.length > 0) {
            const sids = details.map(detail => detail.sid).filter(sid => sid)
            // 获取 creator 和 subject，然后调用 preview 接口获取 subject_name
            const subjectCode = mainData.subjectCode || mainData.subject
            const userId = mainData.creator || mainData.creatorId || mainData.userId
            ...
            if (subjectCode && userId) {
              ...
              const fallbackSubjectName = this.getSubjectName(mainData.subjectCode || mainData.subject) || mainData.subjectCode || mainData.subject
              const requestData = {
                sids: sids,
                subject_name: fallbackSubjectName
              }
    ...
    async confirmDownloadWord() {
      try {
        ...
        const itemResponse = await getPaperAssignment(this.currentDownloadRow.id)
        if (itemResponse.code !== 200) {
          this.$message.error('获取详情失败')
          return
        }

        const mainData = itemResponse.data.main || itemResponse.data
        const details = itemResponse.data.details || []
        ...
        const questionIds = details.map(detail => detail.sid).filter(id => id)
        const questions = await this.getQuestionsData(questionIds, mainData.subjectCode || mainData.subject, mainData.creator)
        ...
        const requestData = {
          html: htmlContent,
          file_name: mainData.customPaperName || '作业文档'
        }
        ...
        if (response instanceof Blob) {
          ...
          link.href = url
          link.download = `${mainData.customPaperName || '作业文档'}_${new Date().getTime()}.docx`
    ...
    handlePreviewConfirm(data) {
      ...
      // 获取科目代码（从 subjectOptions 中查找）
      let subjectCode = ''
      if (subject) {
        const subjectOption = this.subjectOptions.find(opt => opt.subjectName === subject)
        if (subjectOption) {
          subjectCode = subjectOption.subjectCode
        } else {
          // 如果没有找到，尝试直接使用 subject（可能是科目代码）
          subjectCode = subject
        }
      }
      ...
      // 构建 questionDetails 数组
      const questionDetails = selectedQuestions.map(q => {
        const sid = q.sid || q.SID || q.questionSid
        const questionType = q.CateName || q.cateName || q.cate || q.qtype || '未知题型'
        const score = q.score || 0 // 如果没有分值，默认为0
      ...
      // 构建提交数据（新接口格式，固定为作业类型）
      const submitData = {
        subjectCode: subjectCode || subject, // 支持中文科目名，后端会自动转换
      ...
      // 调用新接口
      addPaperAssignment(submitData).then(() => {
        this.$modal.msgSuccess('作业创建成功')
        // 清空已选题目
        this.$store.commit('clearSelectedQuestions')
        // 切换到作业标签页
        this.currentType = 'homework'
        // 重置查询参数并刷新列表
        this.queryParams.pageNum = 1
        this.queryParams.order = '1'
        this.getList()
