<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="文件线上路径" prop="fileOnlinePath">
        <el-input
          v-model="queryParams.fileOnlinePath"
          placeholder="请输入文件线上路径"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="笔记自定义名称" prop="noteCustomName">
        <el-input
          v-model="queryParams.noteCustomName"
          placeholder="请输入笔记自定义名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="noteList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文件线上路径" align="center" prop="fileOnlinePath" />
      <el-table-column label="笔记自定义名称" align="center" prop="noteCustomName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="文件线上路径" prop="fileOnlinePath">
          <el-input v-model="form.fileOnlinePath" placeholder="请输入文件线上路径" />
        </el-form-item>
        <el-form-item label="笔记自定义名称" prop="noteCustomName">
          <el-input v-model="form.noteCustomName" placeholder="请输入笔记自定义名称" />
        </el-form-item>
        <el-form-item label="逻辑删除标识" prop="isDeleted">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fileOnlinePath: [
          { required: true, message: "文件线上路径不能为空", trigger: "blur" }
        ],
        noteCustomName: [
          { required: true, message: "笔记自定义名称不能为空", trigger: "blur" }
        ],
        isDeleted: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户笔记编号为"' + ids + '"的数据项？').then(function() {
        return delNote(ids)
      }).then(() => {
        this.getList()
