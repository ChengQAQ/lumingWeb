  ...
  <div class="app-container card">
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="1220px" class="custom-dialog dialog-center">
      <el-form :model="form" label-width="100px" class="custom-form">
        <el-form-item label="路径" class="wide-path-item">
          <el-cascader
...
<script>
import { dirmap, addDirectoryToDirmap, deleteDirectoryToDirmap, updateDirectoryToDirmap } from '@/api/system/tree'
...
export default {
  ...
  methods: {
    ...
    loadTreeData() {
      ...
      dirmap({}).then(res => {
        // 新接口返回的数据结构：{ code: 200, data: { label: [], treeData: [] } }
        if (res.code === 200 && res.data && res.data.treeData) {
    ...
    loadAllOptions() {
      dirmap({}).then(res => {
        ...
        this.allOptions = this.transformTreeToCascader(treeData)
      })
    },
        ...
        h('span', {
          ...
          h('el-tooltip', { props: { content: '新增', placement: 'top' } }, [
            h('el-button', {
              ...
              on: {
                click: () => this.openDialog('add', data)
              }
            })
          ]),
          h('el-tooltip', { props: { content: '修改', placement: 'top' } }, [
            h('el-button', {
              ...
              style: { borderRadius: '4px', padding: '2px 4px', minWidth: '24px', height: '24px' },
              on: {
                click: () => this.openDialog('edit', data)
              }
            })
          ]),
          h('el-tooltip', { props: { content: '删除', placement: 'top' } }, [
            h('el-button', {
              ...
              style: { borderRadius: '4px', padding: '2px 4px', minWidth: '24px', height: '24px' },
              on: {
                click: () => this.openDialog('delete', data)
              }
            })
          ])
        ])
